...
from tradingagents.agents import (
    create_bear_researcher, create_bull_researcher, create_msg_delete,
    create_market_analyst, create_news_analyst, create_research_manager,
...
class GraphSetup:
    def __init__(self, quick_thinking_llm: Any, deep_thinking_llm: Any, tool_nodes: dict[str, ToolNode], conditional_logic: ConditionalLogic):
        self.quick_thinking_llm, self.deep_thinking_llm, self.tool_nodes, self.conditional_logic = quick_thinking_llm, deep_thinking_llm, tool_nodes, conditional_logic
    ...
    def setup_graph(self, selected_analysts=("market", "sentiment", "news", "onchain", "derivatives", "order_flow", "macro")):
        ...
        for name, factory in {
            "Bull Researcher": create_bull_researcher, "Bear Researcher": create_bear_researcher,
            "Research Manager": create_research_manager, "Trader": create_trader,
            "Aggressive Analyst": create_aggressive_debator, "Neutral Analyst": create_neutral_debator,
            "Conservative Analyst": create_conservative_debator, "Portfolio Manager": create_portfolio_manager,
        }.items():
            workflow.add_node(name, factory(self.deep_thinking_llm) if name in {"Research Manager", "Portfolio Manager"} else factory(self.quick_thinking_llm))
        workflow.add_edge(START, plan.specs[0].agent_node)
        for i, spec in enumerate(plan.specs):
            workflow.add_conditional_edges(spec.agent_node, getattr(self.conditional_logic, f"should_continue_{spec.key}"), [spec.tool_node, spec.clear_node])
            workflow.add_edge(spec.tool_node, spec.agent_node)
            workflow.add_edge(spec.clear_node, plan.specs[i + 1].agent_node if i + 1 < len(plan.specs) else "Bull Researcher")
        for node in ("Bull Researcher", "Bear Researcher"):
            workflow.add_conditional_edges(node, self.conditional_logic.should_continue_debate, DEBATE_PATH_MAP)
        workflow.add_edge("Research Manager", "Trader")
        workflow.add_edge("Trader", "Aggressive Analyst")
        for node in ("Aggressive Analyst", "Conservative Analyst", "Neutral Analyst"):
            workflow.add_conditional_edges(node, self.conditional_logic.should_continue_risk_analysis, RISK_ANALYSIS_PATH_MAP)
